...
<body class="font-sans antialiased text-gray-900">
    {{-- 1. INTELIGÊNCIA NA ABERTURA: Começa fechado se for mobile (768px) --}}
    <div class="flex h-screen bg-gray-100" x-data="{ sidebarOpen: window.innerWidth > 768 }">

        {{-- 2. EFEITO OVERLAY: Sombra que aparece atrás do menu no mobile --}}
        ...
        {{-- 3. COMPORTAMENTO DE POSICIONAMENTO: Fixed no mobile, Relative no desktop --}}
        <aside 
            :class="sidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full w-0 md:w-20 md:translate-x-0'"
            class="fixed inset-y-0 left-0 z-50 transition-all duration-300 bg-gray-900 shadow-xl md:relative flex flex-col overflow-hidden">

            <div class="flex flex-col min-h-[4rem] bg-gray-800/50 text-white overflow-hidden border-b border-gray-700">
                {{-- Container do topo do menu --}}
                <div class="flex items-center justify-between px-4 py-4">
                    <div x-show="sidebarOpen" class="w-full">
                            ...
                            <form action="{{ route('admin.alternar.condominio') }}" method="POST" id="form-contexto">
                                ...
                                <div class="relative">
                                    <select name="condominio_id" onchange="this.form.submit()"
                                        ...
                                        <option value="">🌎 Visão Geral (Master)</option>
                        ...
                        @else
                            <div class="flex items-center space-x-2 py-2">
                                ...
                                <span class="font-bold text-sm uppercase tracking-tight text-indigo-400">
                    ...
                    {{-- 4. BOTÃO DE FECHAR INTERNO (O "X"): Visível apenas no mobile --}}
                    <button @click="sidebarOpen = false" class="md:hidden p-1 text-gray-400 hover:text-white focus:outline-none">
                ...
                <span x-show="!sidebarOpen" class="hidden md:flex items-center justify-center py-4 font-bold text-xl text-indigo-500">
                    {{ session('admin_condominio_id') ? '🏢' : '🌎' }}
                </span>
            </div>

            <nav class="mt-5 px-4 space-y-2 flex-1">
                ...
                <div x-data="{ open: {{ request()->routeIs('apartamentos.*') || request()->routeIs('moradores.*') || request()->routeIs('condominio.*') ? 'true' : 'false' }} }">
                    <button @click="open = !open" class="w-full flex items-center justify-between px-4 py-3 text-gray-400 hover:bg-gray-800 hover:text-white rounded-lg transition-colors duration-200">
                        ...
                        <svg x-show="sidebarOpen" :class="{'rotate-180': open}" class="w-4 h-4 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
                    </button>

                    <div x-show="open && sidebarOpen" x-transition class="mt-2 space-y-1 pl-10">
                        <a href="{{ route('condominio.edit') }}" class="block py-2 text-sm {{ request()->routeIs('condominio.*') ? 'text-white font-bold' : 'text-gray-500 hover:text-white' }}">Condomínio</a>
                        <a href="{{ route('apartamentos.index') }}" class="block py-2 text-sm {{ request()->routeIs('apartamentos.*') ? 'text-white font-bold' : 'text-gray-500 hover:text-white' }}">Unidades</a>
                        <a href="{{ route('moradores.index') }}" class="block py-2 text-sm {{ request()->routeIs('moradores.*') ? 'text-white font-bold' : 'text-gray-500 hover:text-white' }}">Moradores</a>
                    </div>
                </div>
                @endif
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            <header class="h-16 bg-white border-b flex items-center justify-between px-6 shadow-sm">
                {{-- Botão Sanduíche que abre o menu --}}
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button class="text-sm text-red-600 hover:underline font-bold">Sair</button>
                    </form>
                </div>
            </header>
